 ...
 * 
 * TODO: Objective 0: 260632216
 * 
 * @author Jonathan Bernard Bloch 260632216
 */
public class A2PFLApp{
	...
	public void run() {
			...
			System.out.println("Jonathan Bernard Bloch " + Version.getVersion() + "!");	    
			windowInit();
			init();
			loop();
	...
	public void display() {
		...
		//mine #1
		javax.vecmath.Vector3f light = new javax.vecmath.Vector3f(lightPosx.getFloatValue(), lightPosy.getFloatValue(), lightPosz.getFloatValue());
		// this will result in a singular vector if it is gimbal locked
		javax.vecmath.Vector3f right = new javax.vecmath.Vector3f(-lightPosz.getFloatValue(), 0, lightPosx.getFloatValue());
		// singularity :( pick any vector in the plane. Normalize will not work
		if(right.lengthSquared() < 0.01f) right.x = 1;
		right.normalize();
		// up is normalized in gluLookAt https://www.khronos.org/registry/OpenGL-Refpages/gl2.1/xhtml/gluLookAt.xml
		javax.vecmath.Vector3f up = new javax.vecmath.Vector3f();
		up.cross(light, right);
		GLU.gluLookAt(light.x, light.y, light.z, 0, 0, 0, up.x, up.y, up.z);
		...
		//glTranslated(lightPosx.getFloatValue(), lightPosy.getFloatValue(), lightPosz.getFloatValue());
		GLU.gluPerspective((double)lightFOV.getFloatValue(), 1.0, 1.0, 9.0);
		//glTranslated(lightPosx.getFloatValue(), lightPosy.getFloatValue(), lightPosz.getFloatValue());
		//mine1
		...
		// without documentation for tbc

		//GLU.gluLookAt(light.x, light.y, light.z, 0, 0, 0, up.x, up.y, up.z);
		glGetFloatv( GL_PROJECTION_MATRIX, lightProjectionMatrix.asArray() );        
        ...
        glLoadIdentity();
        //GLU.gluLookAt(light.x, light.y, light.z, 0, 0, 0, up.x, up.y, up.z); // also doesn't make any difference
		glMultMatrixf(lightProjectionMatrix.asArray());
		...
		for(int d : data) data[d] = 3000;
		glTexSubImage2D(GL_TEXTURE_2D, 0, /*GL_DEPTH_COMPONENT32,*/ depthFBOSizeWidth/2, depthFBOSizeHeight/2,
				32, 32,  GL_DEPTH_COMPONENT, GL_UNSIGNED_INT, IntBuffer.wrap(data));
